...
export default function DeleteCardCard({
  gameId,
  deckId,
  cardId,
}: {
  ...
  const onDelete = () => {
    ...
    startTransition(async () => {
      try {
        ...
        router.push(`/editor/${gameId}/decks/${deckId}`)
      } catch (error) {
        setErrorMessage("No se pudo eliminar la carta. Intenta de nuevo.")
        console.error("Error deleting card from settings:", error)
      } finally {
        setOpenModal(false)
      }
    })
    ...
    <>
      <section className="rounded-lg border border-red-900/70 bg-red-950/20">
        ...
        <div className="flex flex-col gap-4 px-5 py-5 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-sm font-medium text-slate-100">Eliminar carta</p>
            <p className="mt-1 text-sm text-slate-400">
              Se borraran la carta, sus condiciones, opciones y efectos asociados.
            </p>
          ...
          >
            {isPending ? "Eliminando..." : "Eliminar carta"}
          </button>
        </div>
      </section>
      ...
      <ConfirmModal
        ...
        message="Esto eliminara la carta y todos sus datos asociados. Esta accion no se puede deshacer."
        confirmLabel="Si, eliminar carta"
        onConfirm={onDelete}
